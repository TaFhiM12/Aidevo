...
const SideBar = ({ sidebarOpen, userInfo, user, logOut }) => {
  ...
  const getNavItems = () => {
    ...
    const studentItems = [
      { name: "My Organizations", icon: Users, path: "/dashboard/organizations" },
      { name: "Applications", icon: Bookmark, path: "/dashboard/applications" },
      { name: "Enrolled Events", icon: Calendar, path: "/dashboard/events" },
      { name: "Chat", icon: MessageCircle, path: "/dashboard/chat" },
      { name: "AI Recommendations", icon: Sparkles, path: "/dashboard/recommendations" },
    ];

    const organizationItems = [
      { name: "Events", icon: Calendar, path: "/dashboard/events" },
      { name: "Create Event", icon: Plus, path: "/dashboard/create-event" },
      { name: "Members", icon: Users, path: "/dashboard/members" },
      { name: "Applications", icon: UserPlus, path: "/dashboard/applications" },
      { name: "Chat", icon: MessageCircle, path: "/dashboard/chat" },
    ...
    const superAdminItems = [
      { name: "Organizations", icon: Building, path: "/dashboard/organizations" },
      { name: "Users", icon: Users, path: "/dashboard/users" },
      { name: "System Analytics", icon: BarChart3, path: "/dashboard/analytics" },
      { name: "Reports", icon: Shield, path: "/dashboard/reports" },
    ];
    ...
    switch (userInfo.role) {
      case "student":
        return [...commonItems, ...studentItems, ...profileItems];
      case "organization":
        return [...commonItems, ...organizationItems, ...profileItems];
      case "super-admin":
        return [...commonItems, ...superAdminItems, ...profileItems];
      default:
        return commonItems;
    }
  };
