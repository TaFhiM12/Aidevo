...
const OrganizationProfile = () => {
  ...
  const fetchOrganizationData = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:3000/users/uid/${user.uid}`
      );

      if (response.data.success) {
        const orgData = response.data.user;
  ...
  const updateField = async (field, value) => {
    try {
      ...
      const response = await axios.patch(
        `http://localhost:3000/organizations/${profileData._id}/field`,
        { field, value }
      );
  ...
  const handleImageUpload = async (file, type) => {
    try {
      ...
      console.log(`Starting ${type} upload...`, file);
      ...
      console.log(`${type} uploaded to Cloudinary:`, imageUrl);
  ...
  const handleSaveAll = async () => {
    try {
      ...
      const response = await axios.put(
        `http://localhost:3000/organizations/${profileData._id}/profile`,
        profileData
      );
